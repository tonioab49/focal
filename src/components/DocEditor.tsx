...
export function DocEditor({ filePath, content, title, slug }: DocEditorProps) {
  ...
  const { provider, ydocRef, connectedUsers, user, broadcastSave } = useCollaboration(filePath, {
  ...
  useKeyboardShortcuts(useMemo(() => [{ key: "s", meta: true, handler: handleSave, global: true }], [handleSave]));
  ...
  return (
    <div className="mx-auto max-w-[750px]">
      ...
      {editor && <EditorToolbar editor={editor} className="mb-3 border-b border-gray-200 pb-3" />}
      ...
      {hasChanges && (
        <p className="mt-2 text-xs text-gray-400">
          <kbd className="rounded border border-gray-200 bg-gray-100 px-1 py-0.5 font-mono text-[10px]">⌘S</kbd> to save
        </p>
      )}
    </div>
  );
