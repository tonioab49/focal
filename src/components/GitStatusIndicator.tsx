...
export function GitStatusIndicator({ gitStatus, selectedRepo }: { gitStatus: GitStatus; selectedRepo?: string }) {
  ...
  useKeyboardShortcuts(useMemo(() => [{ key: "Enter", ctrl: true, handler: handleCommit, global: true }], [handleCommit]));
    ...
    <>
      <button
        ...
        className={`flex items-center gap-1.5 rounded-md px-2 py-1 text-xs transition-colors ${
          hasDirty ? "text-amber-700 hover:bg-amber-50 cursor-pointer" : "text-gray-400 cursor-default"
        }`}
        title={hasDirty ? `${fileCount} uncommitted` : "No changes to commit"}
      >
        <span className={`h-2 w-2 rounded-full ${hasDirty ? "bg-amber-500" : "bg-gray-300"}`} />
        <span>{hasDirty ? `${fileCount} uncommitted` : "No changes"}</span>
      </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative rounded-lg border border-gray-200 bg-white shadow-xl w-full max-w-md mx-4 p-5">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-semibold text-gray-900">Uncommitted changes</h2>
                  ...
                  <li key={f.path} className="flex items-start justify-between gap-2">
                    ...
                    <span
                      className={`mt-0.5 shrink-0 rounded px-1.5 py-0.5 text-[10px] font-medium ${
                        f.status === "new" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
                      }`}
                    >
                      {f.status}
                    </span>
              ...
              {result && <p className="mb-3 text-sm text-gray-700">{result}</p>}

              {gitStatus.localMode ? (
                <p className="text-xs text-gray-500 italic">In local mode, commit from your terminal to avoid conflicts with your working tree.</p>
              ) : (
                <button
                ...
                >
                  {isPending ? (
                    ...
                    <>
                      Commit & Push
                      <kbd className="rounded border border-amber-400 bg-amber-500 px-1 py-0.5 font-mono text-[10px] text-amber-100">^&#x21B5;</kbd>
                    </>
                  )}
                </button>
              )}
